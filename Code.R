...
rm(MCD_data) #Removing an unnecessary object from the environment
...
#Defining a function that estimates given ARMA(p,q), returns p-values of coefficients, AIC, and performs the Ljung-Box test. If needed, also plots ACF and PACF
...
arma10 <- arima(MCD_log_returns, order = c(1, 0 , 0))
arch.test(arma10) #Significant arch effects
Box.test(arma10$residuals^2, type = "Ljung-Box", lag = 8) #Confirmed
...
garch11arma10 <- garch_complete(1, 0, 1, 1) #Specification tests OK, Negative sign bias significant

###################
### Final model ###
###################

#Defining event dummies
...
#Final model estimation (TARCH(1,1) ARMA(1,0))
...
log(2)/log(alpha_beta_sum) #5 days volatility halflife
...
addLegend("topleft", on = 1, legend.names = c("Log-returns", "Estimated conditional volatility (baseline model)", "Estimated conditional volatility (no event dummies)"), col = c("black", "red", "blue"), lty = 1, bty = "n", lwd = c(2,1,1))
...
robust_norm_model #Basically the same as final

###################################################
### Auxiliary code used throughout the analysis ###
###################################################
...
#May wanna consider using the Student's distribution given the shape of the log-returns and the repeatedly rejected hypothesis of residuals' normality
